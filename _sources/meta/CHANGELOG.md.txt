...
### 新功能 (ENH)
- 向 `PageObject.extract_text()` 方法添加 `layout_mode_font_height_weight` 参数 (#2920)

### 修复 (BUG)
- 修复 FreeText 注释的字体指定器 (#2893)
- 由于文本行距计算错误，未生成换行符 (#2890)
- 改进文本提取中的空格处理 (#2882)

### 稳定性 (ROB)
- 对错误的 LUT 大小进行软失败处理（flate 编码模式 1） (#2900)

### 文档 (DOC)
- 使用最新的包版本 (#2907)
- 修正读取 FileAttachment 注释的示例 (#2906)

### 开发者体验 (DEV)
- 更新固定的依赖版本 (#2918)
- 使 `make_release.py` 兼容 Windows 环境 (#2894)

### 维护 (MAINT)
- 移除对过时 Python 版本的引用 (#2919)
- 通用化获取 `space_code` 的方法 (#2891)
- 移除不必要的字符映射过程 (#2888)
- 新的 LZW 解码实现 (#2887)
...
### 代码风格 (STY)
- 错误消息首字母大写 (#2903)
- 修改 `PdfWriter` 中的错误消息 (#2902)

[完整更新日志](https://github.com/py-pdf/pypdf/compare/5.0.1...5.1.0)

## 版本 5.0.1，2024-09-29

### 新功能 (ENH)
- 向 `PdfWriter` 构造函数添加 `full` 参数 (#2865)

### 修复 (BUG)
- 更新 `pyproject.toml`，设置最低 Python 版本为 3.8 (#2859)
- 处理字典对象中不平衡的分隔符 (#2878)
- 处理编码中差异过大的情况 (#2873)
- 修复 `extract_text()` 方法中缺少空格的问题 (#1328) (#2868)
- 容忍截断的文件，且跳过 `startxref` 时不发出警告 (#2855)

### 稳定性 (ROB)
- 修复包含无效 Root 对象的 PDF (#2880)
- 当检测到错误时，继续解析字典对象 (#2872)
- 合并包含无效页面的文档到命名目标中 (#2857)
- 容忍数组中的注释 (#2856)

### 开发者体验 (DEV)
- 使用最新 Python 版本进行基准测试 (#2879)

### 维护 (MAINT)
- 向源代码分发包中添加测试 (#2874)
- 重构 `_update_field_annotation` 方法 (#2862)

[完整更新日志](https://github.com/py-pdf/pypdf/compare/5.0.0...5.0.1)
